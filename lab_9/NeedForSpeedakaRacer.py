...
SCORE = 0
COINS = 0
ENEMY_SPEED_INCREASE_COIN_THRESHOLD = 5 # Increase enemy speed after every N coins collected
MAX_SPEED = 10
...
class Coin(pygame.sprite.Sprite):
    def __init__(self, exclude):
        ...
        self.rect = self.image.get_rect()
        self.rect.center = get_random_position(exclude)
        self.weight = random.choice([1, 2, 5])  # Assign a weight randomly from a set of weights
...
while True:
    ...
    if COINS % ENEMY_SPEED_INCREASE_COIN_THRESHOLD == 0 and COINS != 0 and SPEED < MAX_SPEED:
        SPEED += SPEED_INCREASE_AMOUNT
        ENEMY_SPEED_INCREASE_COIN_THRESHOLD += 5  # Increasing threshold to make next speed increase harder
